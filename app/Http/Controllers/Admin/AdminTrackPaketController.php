...
class AdminTrackPaketController extends Controller
{


    public function showTrackForm()
    {
        return view('admin.trackpaket.index');
    }
        ...
        if ($request->has('awb') && $request->has('courier')) {
            ...
            if (in_array($courier, $validCouriers)) {
                ...
                if ($response->successful()) {
                    ...
                    return view('admin.trackpaket.result', ['result' => $result]);
                } else {
                    // Jika respons tidak berhasil (misalnya, nomor Resi salah), kembalikan ke formulir input
                    return redirect()->route('admin.track.form.admin')->with('error', 'Nomor Resi salah atau kurir tidak valid. Silakan coba lagi.');
                }
            } else {
                // Jika kurir yang dipilih tidak valid, kembalikan ke formulir input
                return redirect()->route('admin.track.form.admin')->with('error', 'Kurir yang dipilih tidak valid. Silakan pilih kurir yang sesuai.');
            }
        }

        // Jika tidak ada nomor Resi atau kurir yang diberikan, kembalikan ke formulir input
        return redirect()->route('admin.track.form.admin')->with('error', 'Mohon masukkan nomor Resi dan pilih kurir.');
    }
